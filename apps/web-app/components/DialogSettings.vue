...
export default {
  ...
  methods: {
    forceFullMode () {
      ...
      this.$cookies.set('fullDark', this.fullDark, { maxAge: 34560000 })
      try {
    ...
    switchMergeDuplicates () {
      ...
      this.$cookies.set('mergeDuplicates', this.mergeDuplicates, { maxAge: 34560000 })
      this.delayedFetch()
    },
    setHighlightTeacher () {
      this.highlightTeacher = !this.highlightTeacher
      this.$cookies.set('highlightTeacher', this.highlightTeacher, { maxAge: 34560000 })
      this.delayedFetch()
    },
    reset () {
    ...
    updateBlocklist (event) {
      try {
        this.$cookies.set('blocklist', JSON.stringify(event), { maxAge: 34560000 })
        this.delayedFetch()
      } catch (err) {
      }
    },
    setColor (type, color) {
      try {
        ...
        this.$cookies.set('customColorList', tmpCookie, { maxAge: 34560000 })
      } catch (err) {
        this.$cookies.set('customColorList', { [type]: color }, { maxAge: 34560000 })
      }
      this.delayedFetch()
    }
  }
