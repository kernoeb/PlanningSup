  ...
  >
    ...
    <div class="d-flex justify-space-between">
      <div
        ...
        class="ml-4 mr-4 mb-3 text-truncate"
      >
        <transition
          name="fade"
          mode="out-in"
...
export default {
    ...
    if (this.$cookies.get('plannings')) {
      try {
        ...
        if (oldCookie.startsWith('[') && oldCookie.endsWith(']')) {
          ...
          this.$cookies.set('plannings', j.join(','), { maxAge: 34560000 })
        }
      } catch (err) {
        console.log(err)
      }
    ...
    try {
      ...
      if (oldTzCookie !== undefined) {
        ...
        if (oldTzCookie.newTZ && typeof oldTzCookie.newTZ === 'string') {
          ...
          this.$cookies.set('timezone', oldTzCookie.newTZ, { maxAge: 34560000 })
        }
      }
    } catch (err) {
      console.error(err)
  ...
  watch: {
    '$vuetify.theme.dark' () {
      this.$cookies.set('theme', this.$vuetify.theme.dark ? 'true' : 'false', { maxAge: 34560000 })
    },
    calculateNbHoursComputed: {
      handler (n) {
        if (n) {
