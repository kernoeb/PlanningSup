...
function cleanText(string) {
  return removeAccents(string.replace(/\//g, '').replace(/&/g, '').replace(/,/g, '').replace(/[()]/g, '').replace(/\s/g, '').replace(/\./g, '')).toLowerCase()
}
...
function listener() {
  ...
  if (e.getAttribute('aria-activedescendant') !== null) {
    ...
    let childLevel = Number.parseInt(child.getAttribute('aria-level'))
    const parrentLevel = Number.parseInt(parrent.getAttribute('aria-level'))
    ...
    while (childLevel > parrentLevel && i < 100) {
      ...
      const childId = Number.parseInt(childSubDiv.getAttribute('id').split('_')[1])
      ...
      childLevel = Number.parseInt(child.getAttribute('aria-level'))
      i++
    }

    strClipboard(JSON.stringify(t), (str) => {
      window.alert('Copié dans le presse-papier avec succès')
    })
  }
}
