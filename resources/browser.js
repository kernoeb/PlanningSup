...
function listener() {
    ...
    if (e.getAttribute('aria-activedescendant') !== null) {
        var idTemp = e.getAttribute('aria-activedescendant').split('-')[2]

        var parrent = document.getElementById(`Direct Planning Tree_x-auto-${idTemp}`)
        var parrentSubDiv = parrent.children[0].children[0].children[0].children[1].children[0].children[0]

        var child = parrent.nextSibling
        var childLevel = parseInt(child.getAttribute('aria-level'))
        var parrentLevel = parseInt(parrent.getAttribute('aria-level'))
        var previousChildLevel = parrentLevel

        var folderLabel = parrentSubDiv.children[0].children[4].innerHTML
        var childLabel = folderLabel
        ...
        while (childLevel > parrentLevel && i < 100) {
            ...
            var childId = parseInt(childSubDiv.getAttribute('id').split('_')[1])
            var isFolder = childSubDiv.children[0].children[1].getAttribute('onload') !== null
            if (isFolder) {
                good = false;
                ...
                tTemp.push({
                    "id": cleanText(childLabel),
                    "title": childLabel,
                    "url": `https://planning.univ-ubs.fr/jsp/custom/modules/plannings/anonymous_cal.jsp?resources=${childId}&projectId=0&calType=ical&firstDate=2021-01-01&lastDate=2025-12-31`
                })
            }
        ...
        strClipboard(JSON.stringify(t), function (str) {
            alert("ca a été copié dans le presse-papier");
        });
    }
}
